...
class Trainer():
    ...
    def train(self):
        ...
        for batch, (lr, hr, file_names) in enumerate(self.loader_train):
            ...
            if self.Type=='1*L1_SparseN':

                sr,loss_sparsity = self.model(lr)
                loss = self.loss(sr, hr, loss_sparsity)
                    ...
                    if self.args.test_block:
                        ...
                        for iy in range(0, h, ip):
                            ...
                            for ix in range(0, w, ip):
                                ...
                                if self.Type == '1*L1_SparseN':
                                    sr_p, loss_p = self.model(lr_p)
                                elif self.Type == '1*L1_KL':
                                        sr_p, encodeResult_p,encodeResult_3p,encodeResult_2p,encodeResult_1p = self.model(lr_p)
                                else:
